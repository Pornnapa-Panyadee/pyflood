...
/////////////////////                                                       /////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////
String StationID = "532"; // PYFlood Station ID
//String StationID = "MIXKEY1";  //CRFlood
String StationStatus = "ON";//0968272956
...
//#define magicNumber 1.68  //MagicNumber for RED Phynics Board
#define magicNumber 2.01//MagicNumber for BLUE Phynics Board
...
void loop() {
  ...
  int cur_mil = millis();
  if (cur_mil - prev_led >= 250) {
    prev_led = cur_mil;
    if(analogRead(PIN_ULTRASONIC) <= 0 || analogRead(PIN_ULTRASONIC) >= 1023){
...
void AddData() {
  ...
  if (hasRTC) {
    ...
    date.add(RTC.getDay());
    date.add(RTC.getMonth());
    date.add(RTC.getYear());
    JsonArray& time = data.createNestedArray("time");
    time.add(RTC.getHour());
    time.add(RTC.getMinute());
    time.add(RTC.getSecond());
...
void getRTCdata() {
  ...
  data["date"][0] = RTC.getDay();
  data["date"][1] = RTC.getMonth();
  data["date"][2] = RTC.getYear();

  data["time"][0] = RTC.getHour();
  data["time"][1] = RTC.getMinute();
  data["time"][2] = RTC.getSecond();
